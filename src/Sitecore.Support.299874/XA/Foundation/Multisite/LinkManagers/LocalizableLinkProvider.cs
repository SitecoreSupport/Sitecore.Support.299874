...
{
    public class LocalizableLinkProvider : Sitecore.Support.Links.LinkProvider
    {
        private int _cacheExpiration = 1;

        private const string CacheKey = "LLM_{0}_{1}_{2}_";

        protected IPageMode PageMode
        {
            get;
        } = ServiceLocator.ServiceProvider.GetService<IPageMode>();
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            int.TryParse(config["cacheExpiration"], out _cacheExpiration);
        }
        ...
        public override string GetItemUrl(Item item, UrlOptions options)
        {
            ...
            string text = ServiceLocator.ServiceProvider.GetService<ISiteInfoResolver>().GetSiteInfo(item)?.Name;
            string str = $"LLM_{itemUrl}_{item.Database.Name}_{text}_";
        ...
        protected UrlOptions ResolveLanguageEmbedding(Item item, UrlOptions options)
        {
            ...
            if (siteInfo != null)
            {
                ...
                {
                    options.LanguageEmbedding = ((!(SiteContext.Current.Name != siteContext.Name)) ? LanguageEmbedding.Always : LanguageEmbedding.Never);
                }
            }
            return options;
        }
        ...
        {
            return new LocalizableLinkBuilder(options);
        }
    }
}